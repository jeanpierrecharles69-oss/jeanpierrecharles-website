// =====================================================================
// AEGIS Intelligence -- Gantt Kanban Governance Dashboard v4.0 -- N4
// =====================================================================
// Source PRS : 20260410T1400_SPEC_GANTT-KANBAN-V4-PRS.md
// Auteur scaffold : Claude Code (ACDC) -- N1 (T0000) + N2 (T2225) + N3 (T2200) + N4 (T2240)
// Date scaffold : 20260413T2240 CET
// Statut : DRAFT NOCTURNE N4 -- pre-review Opus Chat matinal 14/04
// Workflow : Claude Code ECRIT / Opus Chat AUDIT V&V / JP+Opus Chat CO-EXEC
// Convention : ASCII strict pour le code (commentaires), accents toleres en strings UI
// IDs definitifs sources : D334 / L235 / R112 (LIFECYCLE v2.9.0 du 06/04)
// N1 base : gantt-kanban-v4-scaffold.jsx (742 lignes)
// N2 ajouts : GanttView SVG + donnees S11.3-S11.7 + ItemDetail FR-04
// N3 ajouts : 7 sprints 3 swimlanes + viewport ms-14/ms-15 + drag visual feedback + bar stacking
// N4 fixes chirurgicaux (audit Opus T1615-T1700 DIVA realigne) :
//   - N3-01 : useMemo visibleSprints+barRows fusionne (deps stables)
//   - N3-02 : champ mort supprime de AppState et initialState
//   - N3-03 : D48 ASCII strict applique aux commentaires (54 em-dash -> --) + 1 arrow -> ->
// =====================================================================
...
interface AppState {
  ...
  sprints: Sprint[];
  items: KanbanItem[];
  // N4 fix N3-02 : champ mort supprime (dead code, jamais lu)
...
// Source : SPEC v4 PRS S11.2 -- 10 sprints connus (N2)
// N3 P1 : +7 sprints (2 reglementaire + 3 sessions-opus + 2 maintenance-ops)
// Ancrage : LIFECYCLE_MASTER v2.9.0 + BRIDGE_STATE.md
// =====================================================================

const SPRINTS: Sprint[] = [
  // === SWIMLANE CODE (7 sprints, inchanges depuis N2) ===
  {
    id: "sp-311",
    version: "v3.1.1",
  ...
  // === SWIMLANE MYTHOS (2 sprints, inchanges depuis N2) ===
  {
    id: "sp-343",
    version: "v3.4.3",
  ...
  // === SWIMLANE COMMERCIAL (1 sprint, inchange depuis N2) ===
  {
    id: "sp-35",
    version: "v3.5",
  ...
  // === SWIMLANE REGLEMENTAIRE (2 sprints, AJOUT N3 P1) ===
  // Source ancrage : LIFECYCLE v2.9.0 section 1.2 fenetres strategiques
  // CRA sept 2026 premiere deadline, AI Act dec 2027 si Omnibus
  {
    id: "sp-reg-cra",
    version: "CRA Art.14",
    ...
    description: "Preparation conformite CRA signalement 24h (deadline 11/09/2026)",
    relatedIds: [],
    commits: [],
  },
  {
    ...
    status: "planned",
    swimlane: "reglementaire",
    description: "Veille AI Act Annexe III (si Omnibus, deadline 02/12/2027)",
    relatedIds: [],
    commits: [],
  },
  // === SWIMLANE SESSIONS-OPUS (3 sprints, AJOUT N3 P1) ===
  // Source ancrage : LIFECYCLE v2.9.0 section 3 + BRIDGE_STATE protocole Night Builder
  // + sessions Opus V&V recurrentes documentees dans les bridges
  {
    id: "sp-opus-lifecycle",
    version: "LIFECYCLE",
    startDate: "2026-03-30",
    endDate: "2026-05-25",
    status: "en-cours",
    swimlane: "sessions-opus",
    description: "Integration LIFECYCLE hebdo (v2.9.0 vers v2.10.0+)",
    relatedIds: [],
    commits: [],
  },
  {
    ...
    status: "en-cours",
    swimlane: "sessions-opus",
    description: "Audits matinaux recurrents V&V (Night Builder + production)",
    relatedIds: [],
    commits: [],
  },
  {
    ...
    status: "en-cours",
    swimlane: "sessions-opus",
    description: "Contre-expertise V&V sprints v3.4.x (P_CR_01)",
    relatedIds: [],
    commits: [],
  },
  // === SWIMLANE MAINTENANCE-OPS (2 sprints, AJOUT N3 P1) ===
  // Source ancrage : BRIDGE_STATE smoke test T2110, aegis-desktop-vv.ps1 v2 (D193),
  // monitoring health deep (R107, L_T2110_01), scripts aegis-ops/
  {
    id: "sp-maint-ps1",
    version: "PS1 maint",
    startDate: "2026-04-06",
    ...
    description: "Maintenance scripts PowerShell aegis-ops/ (hors repo git)",
    relatedIds: [],
    commits: [],
  },
  {
    ...
    status: "planned",
    swimlane: "maintenance-ops",
    description: "Monitoring Mollie/Vercel health deep (R107, L_T2110_01)",
    relatedIds: [],
    commits: [],
  },
];
...
// N3 P2 : GANTT_END etendu +4 semaines pour absorber ms-14 (jan 2027)
// =====================================================================

// Constantes temporelles
const GANTT_START = new Date("2026-03-31"); // Lundi S14
const GANTT_PIVOT = new Date("2026-06-29"); // Lundi S27 -- bascule daily/weekly
// N3 P2 : etendu de "2026-12-28" (S52) a "2027-01-25" (S56, +4 semaines)
// pour absorber ms-14 Machinery Reg 2023/1230 (2027-01-20)
const GANTT_END   = new Date("2027-01-25"); // Lundi S56 -- +4 sem vs N2
...
// =====================================================================
// SECTION 11b -- HELPER BAR STACKING (AJOUT N3)
// Distribue les barres sprint sans chevauchement visuel par swimlane
// Algorithme greedy : affecte le premier rang disponible
// =====================================================================

function computeBarRows(sprints: Sprint[]): Map<string, number> {
  const result = new Map<string, number>();
  const bySwim: Record<string, Sprint[]> = {};
  sprints.forEach(sp => {
    if (!bySwim[sp.swimlane]) bySwim[sp.swimlane] = [];
    bySwim[sp.swimlane].push(sp);
  });

  for (const group of Object.values(bySwim)) {
    group.sort((a, b) => a.startDate.localeCompare(b.startDate));
    const rowEnds: string[] = [];
    for (const sp of group) {
      let row = -1;
      for (let r = 0; r < rowEnds.length; r++) {
        if (sp.startDate > rowEnds[r]) {
          row = r;
          rowEnds[r] = sp.endDate;
          break;
        }
      }
      if (row === -1) {
        row = rowEnds.length;
        rowEnds.push(sp.endDate);
      }
      result.set(sp.id, row);
    }
  }
  return result;
}
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "TOGGLE_SWIMLANE":
      // PLACEHOLDER N4 -- non implemente en N3 par decision perimetre
      return state;
...
// N3 P3 : retour visuel drag-drop (opacite card drag, highlight colonne cible)
// =====================================================================
...
function KanbanView({ items, selectedItemId, onSelectItem, onMoveItem }: KanbanViewProps) {
  // N3 P3 : etat local pour retour visuel drag-drop
      ...
      {COLUMNS.map((col) => {
        ...
        return (
          <div
            key={col.id}
            style={{
              backgroundColor: isDropTarget ? "rgba(59,130,246,0.08)" : P.surface,
              borderRadius: 10,
              border: `1px solid ${isDropTarget ? P.accent : P.border}`,
              overflow: "hidden",
...
// N3 P2 : viewport etendu + indicateur ms-15 hors viewport
// N3 : bar stacking via computeBarRows
// =====================================================================

// --- 15a. GanttAxisX -- labels mois et sprints sur l'axe horizontal ---
// N3 P2 : ajoute "Jan 27" pour couvrir viewport etendu

function GanttAxisX() {
  // Mois a afficher (Avr 2026 a Jan 2027)
...
// N3 : barRow param pour stacking vertical sans chevauchement
// Renomme GanttSprintBar (vs GanttBar du brief) pour eviter collision avec interface GanttBar

function GanttSprintBar({ sprint, barRow, dispatch }: { sprint: Sprint; barRow: number; dispatch: React.Dispatch<AppAction> }) {
  ...
  const barY = swimlaneToY(idx) + BAR_Y_OFFSET + barRow * (BAR_HEIGHT + BAR_GAP);
...
// --- 15f. GanttOverflowIndicator -- indicateur jalons hors viewport (N3 P2 Option B) ---
// Affiche un badge a la bordure droite du Gantt listant les milestones hors viewport

function GanttOverflowIndicator({ milestones }: { milestones: Milestone[] }) {
  if (milestones.length === 0) return null;
  const rightX = GANTT_WIDTH - 30;
  const y = GANTT_PADDING_TOP - 20;
  const label = milestones.length === 1
    ? `-> 1 jalon hors viewport`
    : `-> ${milestones.length} jalons hors viewport`;
  const tooltipLines = milestones.map(m => `${m.label} — ${m.date} — ${m.criticality}`).join("\n");
  ...
  return (
    <g style={{ cursor: "help" }}>
      <rect
        ...
        strokeOpacity={0.4}
      />
      <text x={rightX} y={y} fill={P.rose} fontSize={9} fontWeight="600">
        {label}
      </text>
...
// --- 15g. GanttView -- composant principal vue Gantt ---
// Source : SPEC v4 PRS FR-07 a FR-12
// N3 : bar stacking + viewport etendu + indicateur overflow

function GanttView({ state, dispatch }: { state: AppState; dispatch: React.Dispatch<AppAction> }) {
  const todayX = dateToX(new Date());

  // N4 fix N3-01 : visibleSprints et barRows memoized ensemble, deps stables
  // SPRINTS et GANTT_START sont des constantes module-level, donc deps vide OK
  const { visibleSprints, barRows } = useMemo(() => {
    const vs = SPRINTS.filter((sp) => new Date(sp.endDate) >= GANTT_START);
    const br = computeBarRows(vs);
    return { visibleSprints: vs, barRows: br };
  }, []);

  // N3 P2 : filtrer milestones dans le viewport etendu (date <= GANTT_END)
  // ms-14 (jan 2027) maintenant visible, ms-15 (dec 2027) reste hors viewport
  const visibleMilestones = MILESTONES.filter((ms) => {
    const d = new Date(ms.date);
    return d >= GANTT_START && d <= GANTT_END;
  });

  // N3 P2 : milestones hors viewport pour indicateur Option B
  const overflowMilestones = MILESTONES.filter((ms) => {
    const d = new Date(ms.date);
    return d > GANTT_END;
  });
  ...
  return (
    <div style={{ overflowX: "auto", backgroundColor: P.bg, padding: "16px" }}>
      ...
      >
        ...
        {/* Couche 3 : barres sprint colorees par couloir -- N3 bar stacking */}
        {visibleSprints.map((sprint) => (
          <GanttSprintBar
            key={sprint.id}
            sprint={sprint}
            barRow={barRows.get(sprint.id) ?? 0}
            dispatch={dispatch}
          />
        ))}
        ...
        {/* Couche 7 : indicateur milestones hors viewport (N3 P2 Option B) */}
        <GanttOverflowIndicator milestones={overflowMilestones} />
      </svg>
    </div>
  );
}
...
// =====================================================================
// SECTION 17 -- PLACEHOLDERS VUES N4
// IdsView, MetricsView, ExportView -- a implementer N4
// =====================================================================

function IdsView() {
  return (
    <div style={{ padding: 32, color: P.muted, fontSize: 13, textAlign: "center" }}>
      Vue IDs — a implementer en Night Builder N4 (13-14/04)
    </div>
  );
}
...
function MetricsView() {
  return (
    <div style={{ padding: 32, color: P.muted, fontSize: 13, textAlign: "center" }}>
      Vue Metriques — a implementer en Night Builder N4 (13-14/04)
    </div>
  );
}
...
function ExportView() {
  return (
    <div style={{ padding: 32, color: P.muted, fontSize: 13, textAlign: "center" }}>
      Vue Export — a implementer en Night Builder N4 (13-14/04)
    </div>
  );
}
...
// N3 : sprints 17 total, viewport etendu, drag visual feedback
// =====================================================================
...
const initialState: AppState = {
  ...
  // N4 fix N3-02 : champ mort [] supprime (dead code)
  ...
  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", backgroundColor: P.bg, color: P.text, minHeight: "100vh" }}>
      ...
      <footer style={{ padding: "12px 24px", borderTop: `1px solid ${P.border}`, fontSize: 10, color: P.muted, fontFamily: "monospace" }}>
        v4.0 N4 — Regime D10r2 — Convention D48 — {TODAY}
      </footer>
    </div>
  );
}
